...
interface ShareButtonProps {
  ...
  /** Icon-only, no outline; for narrow viewports */
  compact?: boolean
}
...
export function ShareButton({
  ...
  const description = t("checkOut", { name: title })
  ...
  const copyToClipboard = () => {
    navigator.clipboard
      ...
      .then(() => {
        ...
        toast.success(
          <div>
            {t("copiedLink")} <code>{shareUrl}</code>
          </div>,
        )
  ...
  const shareViaEmail = () => {
    ...
    toast(t("emailOpened"), {
      description: t("emailOpenedDescription"),
    })
  }
    ...
    return (
      <DropdownMenuItem asChild>
        <Button variant="dropdown-item" onClick={handleShare}>
          <span className="w-full">{t("share")}</span>
          <ShareIcon />
        </Button>
      </DropdownMenuItem>
    )
        ...
        {isWebShareAvailable ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-lg" onClick={handleShare} aria-label={t("ariaShare")}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon-lg" aria-label={t("ariaShare")}>
                <ShareIcon className="size-5!" />
              </Button>
            </DropdownMenuTrigger>
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button variant={variant} size={size} onClick={handleShare}>
              ...
              <span className="block md:hidden">{t("share")}</span>
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>{t("tooltip")}</p>
