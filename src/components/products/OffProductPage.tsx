...
import {
  OffPageNotTrackedRichLink,
  OffPageSparseDataRichLink,
  OffProductBarcodeProvider,
} from "@/components/products/off-product-rich-links"
...
export function OffProductPage({ product, barcode, children }: OffProductPageProps) {
  ...
  return (
    <OffProductBarcodeProvider barcode={barcode}>
      <div className="mx-auto mb-8 flex w-full max-w-[1320px] flex-col px-4 pt-4 lg:py-4">
        ...
        <Callout variant="warning" icon={InfoIcon} className="mb-4 w-full max-w-full">
          <p className="text-sm">
            {t.rich("notTracked", {
              link: OffPageNotTrackedRichLink,
            })}
          </p>
        </Callout>
        ...
        <div className="mb-2 flex items-center justify-between md:mb-3">
          <nav
            ...
            className="flex items-center gap-0.5 overflow-hidden text-xs md:text-sm"
          >
            <Link
              href="/products"
...
function SparseDataNote() {
  const t = useTranslations("products.offPage")
  return (
    <p className="text-muted-foreground mt-1 text-xs leading-relaxed">
      {t.rich("sparseData", {
        link: OffPageSparseDataRichLink,
      })}
    </p>
  )
}
