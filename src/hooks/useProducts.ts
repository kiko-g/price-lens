...
// Extended store products hook with pagination response
type StoreProductsGridParams = {
  page: number
  limit: number
  query?: string
  searchType?: string
...
type StoreProductsGridResponse = {
  data: StoreProduct[]
  pagination: {
    page: number
    limit: number
...
async function getStoreProductsForGrid(params: StoreProductsGridParams): Promise<StoreProductsGridResponse> {
  const filteredParams = Object.fromEntries(
    Object.entries(params).filter(([, v]) => v !== undefined && v !== null && v !== ""),
  )
  const response = await axios.get("/api/products/store", { params: filteredParams })
  if (response.status !== 200) {
    throw new Error("Failed to fetch products")
  }
  return response.data as StoreProductsGridResponse
}

export function useStoreProductsGrid(
  params: StoreProductsGridParams,
  options?: { enabled?: boolean; initialData?: StoreProductsGridResponse },
) {
  return useQuery({
    queryKey: [
      "storeProductsGrid",
    ...
    queryFn: () => getStoreProductsForGrid(params),
