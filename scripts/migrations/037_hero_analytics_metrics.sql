-- Migration: Add hero-facing metrics to compute_analytics_snapshot.
-- 1. total_savings_euros_24h in price_intelligence (sum of price decreases in EUR)
-- 2. on_discount count per store in per_store array
...
BEGIN
  ...
  sp_counts AS (
    SELECT
      ...
      coalesce(round(sum(
        CASE WHEN available = true
              AND discount IS NOT NULL AND discount > 0
              AND price_recommended IS NOT NULL AND price IS NOT NULL
              AND price_recommended > price
        THEN price_recommended - price
        ELSE 0 END
      )::numeric, 2), 0)::NUMERIC AS total_discount_savings_euros,
      ...
      count(*) FILTER (WHERE scraped_at >= now() - interval '24 hours')::INT AS fresh_within_24h,
      count(*) FILTER (WHERE scraped_at >= now() - interval '48 hours'
                        AND scraped_at <  now() - interval '24 hours')::INT AS fresh_24h_48h,
      count(*) FILTER (WHERE scraped_at >= now() - interval '72 hours'
                        AND scraped_at <  now() - interval '48 hours')::INT AS fresh_48h_3d,
      count(*) FILTER (WHERE scraped_at >= now() - interval '168 hours'
                        AND scraped_at <  now() - interval '72 hours')::INT AS fresh_3d_7d,
      count(*) FILTER (WHERE scraped_at >= now() - interval '336 hours'
                        AND scraped_at <  now() - interval '168 hours')::INT AS fresh_7d_14d,
      count(*) FILTER (WHERE scraped_at <  now() - interval '336 hours')::INT AS stale_14d,
  ...
  per_store AS (
    ...
    FROM (
      SELECT
        ...
        count(*) FILTER (WHERE sp.available = true AND sp.discount IS NOT NULL AND sp.discount > 0)::INT AS on_discount
  ...
  price_changes AS (
    SELECT
      ...
      coalesce(round((sum(prev.price - p_new.price) FILTER (WHERE p_new.price < prev.price))::numeric, 2), 0)::NUMERIC AS total_savings_euros
    FROM prices p_new
    LEFT JOIN LATERAL (
      SELECT price
      FROM prices pp
  ...
  SELECT jsonb_build_object(
    ...
    'scrape_freshness', jsonb_build_object(
      'within_24h', c.fresh_within_24h,
      'within_48h', c.fresh_24h_48h,
      'within_3d', c.fresh_48h_3d,
      'within_7d', c.fresh_3d_7d,
      'within_14d', c.fresh_7d_14d,
      'older_14d', c.stale_14d,
      'never', c.never_fresh
    ),
    ...
    'price_intelligence', jsonb_build_object(
      ...
      'total_savings_euros_24h', pc.total_savings_euros,
      'total_discount_savings_euros', c.total_discount_savings_euros
    ),
